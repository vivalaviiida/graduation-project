...
dataloader = loadImgs(batchSize=20)
...
for epoch in range(niter):
    for i,(img_data,img_label) in enumerate(dataloader): 
        ...
        EG_loss = EG_L1_loss + 0.0001*G_img_loss + 0.01*Ez_loss + G_tv_loss+ 0.006*VGG_L1_loss
        reset_grad()
        EG_loss.backward()
    ...
    msg5 = format("G_tv_loss:%f"%(G_tv_loss.item()),"<30")+"|"+format("Ez_loss:%f"%(Ez_loss.item()),"<30")\
    +"|"+format("VGG_loss:%f"%(VGG_L1_loss.item()))
