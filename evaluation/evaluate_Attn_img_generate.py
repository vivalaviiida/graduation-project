...
Encoder_Attn = models_lowAttn.Encoder_Plus()
...
Generator_Attn = models_lowAttn.Generator()
...
# if True:
    ...
    Encoder_Attn = Encoder_Attn.cuda()
    ...
    Generator_Attn = Generator_Attn.cuda()
...
Encoder_Attn.load_state_dict(torch.load(Attn_E_path))
...
Generator_Attn.load_state_dict(torch.load(Attn_G_path))
...
for i in range(20):
    ...
    z_Attn = Encoder_Attn(img_data_v)
    ...
    fake_Attn = Generator_Attn(z_Attn, age_v, img_gender_v)
    ...
    for j in range(batchSize):
        ...
        vutils.save_image(fake_Attn[j].data, '%s/Attn%03d.png'%(outf, i*batchSize+j), normalize=True )
